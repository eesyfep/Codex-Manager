...
이 fork는 공개 프로젝트를 이어서 관리합니다. 남겨 둔 출처 링크는 GitHub 저장소 하나입니다: <https://github.com/qxcnm/Codex-Manager>.

## Fork 업데이트

- 모델 라우팅, Aggregate API, Codex App 모델 표시, Claude Code tool-chain 호환성을 여러 차례 수정했습니다.
- 요청 로그에 세션 컨텍스트, ToolSearch degrade 모드, tool schema 검증 요약을 추가했습니다.
- 대시보드, 비용 추정, 서드파티 모델 가격, token/cache 추세, 히스토리 통계를 보강했습니다.
- 자세한 내용은 [변경-이력.md](변경-이력.md)를 확인하세요.
